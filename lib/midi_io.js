...
export class MIDI_IO{
	...
	static webMidiEnable(err) {
        ...
        MIDI_IO.map = new PremadeSongMap("Let it Be");
        MIDI_IO.initDropdowns();
    }

    static updateSongTitlesDropdown(selector, data){
        $(selector).empty();
        $('<option/>', {
                value: "None",
                html: "None selected"
    ...
    static initDropdowns(){
        ...
        MIDI_IO.updateSongTitlesDropdown('#songTitlesSelect', MIDI_IO.map.songTitles());
        ...
        $('#songTitlesSelect').change(function() {
            MIDI_IO.map.song = ($(this).val());
        });
    }
